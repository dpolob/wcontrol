...
def cyclic(column_date: pd.DataFrame) -> tuple:
    ...
    df['dia'] = column_date.apply(lambda x: x.day / 31)
    df['mes'] = column_date.apply(lambda x: x.month / 12)
    df['hour'] = column_date.apply(lambda x: x.hour / 24)


    return (df['dia'], df['mes'], df['hour'])
...
def generarvariablesZmodel(estaciones: list=None, escaladores: list=None, outliers: list=None) -> tuple:
    ...
    for estacion in tqdm(estaciones, desc='Estaciones'):
        ...
        if not check_longitud(df, 25000):
            continue
