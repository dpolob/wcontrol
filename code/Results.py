...
def zmodel(file):
    print(Fore.BLUE + "RESULTADOS MODELO ZONAL" + Style.RESET_ALL)
    ...
    try:
        ...
        print(f"\t\t\t\t\t\t{OK}")
        print(f"\tDatos de predicciones de zona {Path(cfg.paths.zmodel.predictions)}", end="")    
    ...
    print(Fore.YELLOW + "Generando gráficos" + Style.RESET_ALL, end="")
    if y_pred.ndim == 1:  ## N es variable por lo que se almacena como objetos y su shape es (len(test))
        for idx in tqdm(muestras):
            for prediccion in ['hr', 'temperatura', 'precipitacion']:
    ...
    else:  # y_pred es (len(test), N, Ly, Fout)
        for idx in tqdm(muestras):
            for prediccion in ['hr', 'temperatura', 'precipitacion']:
                if prediccion == 'temperatura':
                    plt.plot(np.mean(y_pred[idx, ..., 0], axis=0).reshape(-1,1), 'red')
...
def pmodel(file, temp, hr, rain):
    ...
    if not temp and not hr and not rain:
        print("No se ha definido que mostrar. Ver pmodel --help")
        exit()
